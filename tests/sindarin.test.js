...
*/
  ...
  it('05800 - sandhi master switch (no transformation)', () => {
    // The master switch rule no longer applies transformations - it just acts as a toggle
    // for all sandhi rules (05801-05849). Tests for individual sandhi rules are in the
    // "05800 - sandhi changes" describe block above.
    expect(sindarinRules['3868328117'].mechanic('abc').out).toBe('abc');
    expect(sindarinRules['3868328117'].mechanic('elmbereθ').out).toBe('elmbereθ');
    expect(sindarinRules['3868328117'].mechanic('findrod').out).toBe('findrod');
    expect(sindarinRules['3868328117'].mechanic('gondndor').out).toBe('gondndor');
    expect(sindarinRules['3868328117'].mechanic('milmbar').out).toBe('milmbar');
    expect(sindarinRules['3868328117'].mechanic('morŋgoθ').out).toBe('morŋgoθ');

    // Morphemes are preserved unchanged
    const compoundA = sindarinRules['3868328117'].mechanic('elmbereθ', { morphemes: ['elm', 'bereθ'] });
    expect(compoundA.out).toEqual('elmbereθ');
    expect(compoundA.morphemes).toEqual(['elm', 'bereθ']);
  });
